...
│   ├── model_data_knn_before_encoding.csv
│   ├── model_data_knn_after_encoding.csv.gz
...
The large raw CRMLS files and intermediate model matrices may be stored outside version control.
...
```bash
pip install pandas numpy matplotlib seaborn scipy scikit-learn joblib xgboost geopandas jupyter streamlit
```

### 3. Prepare the Raw Data

Place the monthly CRMLS sold-property files in:

```text
data/all_sales_data/
```

Expected filenames follow this format:
...
Then run the project notebooks in sequence:

```text
Week2/01_exploration.ipynb
Week3/02_preprocessing.ipynb
Week4/03_baseline_model.ipynb
Week5/04_model_comparison.ipynb
Week6/05_advanced_models.ipynb
Week7/06_evaluation.ipynb
```

The preprocessing notebook creates the cleaned and encoded modeling datasets and saves the reusable preprocessing artifacts.

The advanced-model notebook expects the final feature-engineered matrices under:

```text
data/week6_final_model_data/
```

If reproducing the project from the raw source, generate the Week 6 engineered feature matrices before running the XGBoost notebook.

---

## Launching the Streamlit App

The Week 9 application is an optional Streamlit prediction interface. It accepts core property characteristics such as:
...
If `app.py` is included in the project root, launch it from the project directory with:
...
The app should load the saved preprocessing/model artifacts with `joblib` or `pickle` and apply the same preprocessing used during training before generating a predicted close price.
